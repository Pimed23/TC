    ...
    while(chk) {
        ...
        for(int i=0; i<part.size(); i++) {
            for(int j=0; j<2; j++) {
                ...
                for(size_t k=0; k<part[i].size(); k++) {
...
Automata getAutomata( QString qs ) {
    ...
    regexp = qs.toLocal8Bit().constData();
    ...
    vector<QString> estados;
    for( int i = 0; i < (int)min_dfa.size(); i++ ) {
        QString name = "S";
        name = name + QString::number(i);
        estados.push_back( name );
    }

    // OBTENIENDO EL ALFABETO //
    vector<QString> alfabeto;
    alfabeto.push_back( "a" );
    alfabeto.push_back( "b" );

    // OBTENIENDO EL ESTADO INICIAL //
    int start_st = min_dfa_tmp.first;
    QString sInicial = "S" + QString::number( start_st );

    // OBTENIENDO LOS ESTADOS FINALES //
    vector<QString> sFinal;
    for( int i = 0; i < (int)min_dfa.size(); i++ ) {
        if( get<2>( min_dfa[ i ]))
            sFinal.push_back( "S" + QString::number( i ));
    }

    // OBTENIENDO LAS TRANSICIONES //
    vector<vector<QString>> T;
    vector<QString> help;
    for( size_t i = 0; i < estados.size(); ++i ) {
        help.push_back("S" + QString::number(get<0>( min_dfa[ i ] )));
        help.push_back("S" + QString::number(get<1>( min_dfa[ i ] )));
        T.push_back( help );
        help.clear();
    }

    qDebug() << T;
    Automata A( estados, alfabeto, sFinal, sInicial, T );
    return A;
}
